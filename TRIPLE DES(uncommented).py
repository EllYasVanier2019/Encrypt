...
def Sbox(inputmessage):
    ...
    for boxnum in range(8):  # Substitution Box
        ...
        temp = op.inttobin(ColumnNumberTables[boxnum][columnrowindex])[4:]
...
def DESencrypt(input, Keys, round):
    ...
    for index in range(blocks):
        ...
        for subkey in range (16):
            ...
            righttemp = op.bitxor(Keys[round][subkey],righttemp)
...
def D3DES(message, ikey):
    message = op.bintostring(op.hextobin(message))
    keys = keyCreator(ikey)

    return (DESdecrypt(DESencrypt(DESdecrypt(message, keys, 2), keys, 1), keys, 0))



message = "hello"
key = "123456788765432114785236"

print (E3DES(message,key))
print (D3DES(E3DES(message,key), key))
