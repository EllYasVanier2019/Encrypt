...
class TripleDES():
    def __init__(self, plainkey =input("Enter key: ")):
        self.key = self.keyCreator(plainkey)

    def keyCreator(self, plainkey):
    ...
    def Sbox(self, inputmessage):
        ...
        # the tables that are used to get the output message
        self.ColumnNumberTables = [[14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7,
        ...
        for boxnum in range(8):  # Substitution Box
            ...
            temp = op.inttobin(self.ColumnNumberTables[boxnum][columnrowindex])[4:]
    ...
    def DESencrypt(self, input, Keys, round):
        ...
        for index in range(blocks):
            ...
            message = self.Perm(InitialPerm, message)
            ...
            for subkey in range(16):
                ...
                # expands righttemp
                righttemp = self.Perm(Expansionbit, MessageRight)
                ...
                righttemp = self.Perm(SecondPerm,righttemp)  # runs perm function on right temp
            ...
            for g in range(64):
                temp += message[Finalperm[g] - 1]  # final permutation
    ...
    def DESdecrypt(self, input, Keys, round):
        ...
        for index in range(blocks):
            ...
            for subkey in range (16):
                ...
                righttemp = op.bitxor(Keys[round][15 - subkey],righttemp) # XOR the keys in inverse order with right temp
    
                while len(righttemp) < 48: # while xor-ing the subkey and righttemp, the length of the final message may be lower than 48 due to no padding from the function
                    righttemp = "0" + righttemp
    
                righttemp = self.Sbox(righttemp) # runs the sbox function on right temp
    
                righttemp = self.Perm(SecondPerm,righttemp) # run perm fucntion on right temp
            ...
            for g in range(64):
                temp += message[Finalperm[g] - 1] # does the final permutaion
    ...
    def E3DES(self, message):
        print (op.bintohex(op.stringtobin(self.DESencrypt(self.DESdecrypt(self.DESencrypt(message, self.key, 0), self.key, 1), self.key, 2))))

        return (op.bintohex(op.stringtobin(self.DESencrypt(self.DESdecrypt(self.DESencrypt(message, self.key, 0), self.key, 1), self.key, 2))))

    def D3DES(self, message):
        message = op.bintostring(op.hextobin(message))

        return (self.DESdecrypt(self.DESencrypt(self.DESdecrypt(message, self.key, 2), self.key, 1), self.key, 0))
...
a = TripleDES()
print(a.D3DES(a.E3DES(input("Enter Message: "))))
